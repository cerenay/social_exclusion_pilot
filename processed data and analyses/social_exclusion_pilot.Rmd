...
```{r}
sankey_dg<-rbind(treatment, control)%>%
  mutate(all="All")%>%
  ...
  mutate(dict=ifelse(dg<5, "<5", ifelse(dg==5, "5", ">5")))%>%
  dplyr::select(all, condition, dict )

hchart(data_to_sankey(sankey_dg), "sankey")%>%
hc_title(text= "Sent amounts in DG by treatment") %>%
hc_caption(text = "<b>Prolific sample for both treatment and control.<b>")%>%
...
sankey_dg<-rbind(treatment, control)%>%
  ...
  mutate(dg=ifelse(role_dg==1, group_sent, send_proxy))%>%
  mutate(dict=ifelse(dg<5, "<5", ifelse(dg==5, "5", ">5")))%>%
  dplyr::select(all, condition,fair, dict )

hchart(data_to_sankey(sankey_dg), "sankey")%>%
hc_title(text= "Sent amounts in DG by treatment and group distribution") %>%
hc_caption(text = "<b>Prolific sample for both treatment and control.<b>")%>%
hc_add_theme(hc_theme_economist())
```
...
treatment %>% 
  ...
  labs(title= "Gini coefficients for all groups",
       subtitle = "54 groups in total, dashed line is mean gini")+
